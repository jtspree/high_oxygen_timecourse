...
def parse_vO2_ECS_DIRK_oxidation_data(folder):
    ...
    vO2_ECS_DIRK_oxidation_data.drop([1, 2], axis=1, inplace=True)
    vO2_ECS_DIRK_oxidation_data.columns = ['Time', 'Delta']
    vO2_ECS_DIRK_oxidation_df = pd.DataFrame(vO2_ECS_DIRK_oxidation_data)

    vO2_ECS_DIRK_oxidation_df['x_correct'] = vO2_ECS_DIRK_oxidation_df['Time'] - vO2_ECS_DIRK_oxidation_df['Time'].iloc[249]
    vO2_ECS_DIRK_oxidation_df['y_correct'] = vO2_ECS_DIRK_oxidation_df['Delta'] - vO2_ECS_DIRK_oxidation_df['Delta'].iloc[239:249].mean(axis=0)
    vO2_ECS_DIRK_oxidation_df['y_initial'] = vO2_ECS_DIRK_oxidation_df['y_correct'].iloc[239:249].mean(axis=0)
    vO2_ECS_DIRK_oxidation_df['y_final'] = vO2_ECS_DIRK_oxidation_df['y_correct'].iloc[280:300].mean(axis=0)
...
def vO2_ECS_DIRK_oxidation_rates_calc(vO2_ECS_DIRK_oxidation_df, sample, timepoint, rep, destination_folder):
    vO2_ECS_DIRK_oxidation_slope = pd.DataFrame(columns=['x_initial', 'x_final', 'y_initial', 'y_final'])
    for x in range(2, 7):
        rates_dict = {}
        rates_dict['x_initial'] = vO2_ECS_DIRK_oxidation_df['x_correct'][249]
        rates_dict['x_final'] = vO2_ECS_DIRK_oxidation_df['x_correct'][249 + x]
        rates_dict['y_initial'] = vO2_ECS_DIRK_oxidation_df['y_correct'].iloc[239:249].mean(axis=0)
        rates_dict['y_final'] = vO2_ECS_DIRK_oxidation_df['y_correct'].iloc[249 + x]
...
def save_vO2_ECS_DIRK_oxidation_plot(destination_folder, basename, vO2_ECS_DIRK_oxidation_df, vO2_ECS_DIRK_oxidation_mean):
    ...
    y4 = vO2_ECS_DIRK_oxidation_mean * x + vO2_ECS_DIRK_oxidation_df['y_initial'][0]
    ax.set_xlim(-0.1, 0.2)
