...
def save_flr_measurement_plot(master_df, root_master_folder, figure_rows, figure_columns):
    ...
    for timepoint in figure_rows:
        ...
        for sample in figure_columns:
            ...
            ax = fig.add_subplot(len(figure_rows), len(figure_columns)+1, i)
            i = i + 1
            ...
            if sample == figure_columns[0]:
                ...
                ax.set_ylabel('Fluorescence Parameters', fontsize=10)
            else:
                ax.set_yticks([])
                ax.set_yticklabels([])
            ...
            for index, key in enumerate(box_order):
                ...
                ax.text(x + 0.5 * width, y_lim[1] - 0.25, box_specs[1],
                        ...
                        fontsize=9, color=box_specs[2])

            ax.text(x_lim[0] + (x_lim[1] - x_lim[0])*0.02, y_lim[0] + (y_lim[1] - y_lim[0])*0.02,
                    'High Oxygen Time (' + str(timepoint) + ' hr)', color='black', fontsize=10)

            if (sample == figure_columns[-1]) and (timepoint == figure_rows[0]):
                ax.legend(loc='upper left', bbox_to_anchor=(1, 1), prop={'size': 15}, title='Legend')
